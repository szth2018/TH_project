...
const { webpack } = require('webpack')//把网页托管到内存中去
const webpack1 = require('webpack')//使用Jquery之前需要定义此项
...
module.exports = {
  ...
  plugins: [ // 插件配置节点
    ...
    new webpack1.ProvidePlugin({ //jquery插件全局配置$
      $: "jquery",
      jQuery: "jquery"
     })
  ],
  ...
  module: { // 用来配置 非JS文件对应的loader的
    rules: [ // 就是这些 非 JS 文件 和 loader 之间的对应关系
      ...
      {test: /\.ttf|woff|eot|svg$/,use:'url-loader'},//处理样式中字体文件路径的问题
    ]
  },
  resolve:{
    /* alias:{//别名
